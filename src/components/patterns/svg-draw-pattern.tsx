    ...
    <header className="relative border-b border-orange-500/20 bg-zinc-900/50 backdrop-blur-sm">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 md:py-24">
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white tracking-tight uppercase mb-6">
          <span className="text-orange-500">SVG</span> Draw
        </h1>

        {/* Description */}
        <p className="text-zinc-400 text-lg md:text-xl max-w-3xl font-light leading-relaxed">
          Animate SVG paths to draw themselves on scroll. The stroke-dasharray and stroke-dashoffset technique
          creates a mesmerizing self-drawing effect that works on any SVG path, from simple lines to complex illustrations.
        </p>
        ...
        <div className="flex flex-wrap gap-4 mt-8">
          <div className="flex items-center gap-2 px-4 py-2 bg-zinc-800/50 border border-zinc-700/50 rounded">
            ...
            <span className="text-zinc-300 text-sm font-mono">strokeDasharray</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-zinc-800/50 border border-zinc-700/50 rounded">
            <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse" />
            <span className="text-zinc-300 text-sm font-mono">strokeDashoffset</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-zinc-800/50 border border-zinc-700/50 rounded">
            <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse" />
            <span className="text-zinc-300 text-sm font-mono">getTotalLength()</span>
          </div>
        </div>
      </div>
    </header>
        ...
        <div className="relative bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden">
          ...
          <pre className="p-6 pt-8 overflow-x-auto">
            <code className="text-sm font-mono leading-relaxed">
              ...
              <span className="text-zinc-500">{`// SCROLLTRIGGER FADE REVEAL PATTERN`}</span>
              <span className="text-zinc-500">{`// ============================================================================`}</span>
              <br />
              <br />
              ...
              <span className="text-yellow-300">{` FadeRevealComponent`}</span>
              <span className="text-white">() {`{`}</span>
              <br />
              ...
              <span className="text-white">(elements, {`{`}</span><span className="text-white">{` opacity: 0, y: 60 `}</span><span className="text-white">{`}`});</span>
              <br />
              <br />
              &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-zinc-500">{`// Create staggered fade reveal animation`}</span>
              <br />
              ...
              <span className="text-cyan-400">{`className="space-y-4"`}</span>
              <span className="text-purple-400">{`&gt;`}</span>
              <br />
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span className="text-purple-400">{`&lt;div`}</span>
              <span className="text-white">{` className="animate-item bg-zinc-800 p-6 rounded" `}</span>
              <span className="text-purple-400">{`&gt;`}</span>
...
function PatternNotes() {
  const notes = [
    {
      title: 'GET TOTAL LENGTH',
      description: 'The getTotalLength() method returns the exact pixel length of any SVG path. This is essential for the dasharray/dashoffset technique to work correctly.',
    },
    {
      title: 'STROKE DASHARRAY',
      description: 'Setting strokeDasharray to the path length creates a dashed stroke where the dash equals the entire path. This is the "canvas" for the drawing animation.',
    },
    {
      title: 'STROKE DASHOFFSET',
      description: 'Animates from length (invisible) to 0 (fully drawn). The offset shifts the dash, creating the illusion that the path is being drawn.',
    },
    {
      title: 'SCRUB ANIMATION',
      description: 'Using scrub: 1 links the drawing progress to scroll position. Users can "draw" the SVG by scrolling, with automatic reversal on scroll up.',
    },
  ];
